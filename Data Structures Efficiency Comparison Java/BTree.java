...
public class BTree<E extends Comparable<E>> implements SearchTree<E> {
    ...
    private int binarySearch(E item, E[] root, int start, int finish) {
        ...
        if (item.equals(root[middle]) ) {
            return middle;
        }
        else if (item.compareTo(root[middle]) < 0) {
            return binarySearch(item, root, start, middle-1 );
        }
        else {
            return binarySearch(item, root, middle + 1, finish-1);
        }


    }

    public int findx(E item) {
        return binarySearch(item, root.data, 0, root.size);
    }
    /*</listing>*/
